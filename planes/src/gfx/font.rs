...
impl Font {
    pub fn get_char(&self, char: char) -> Vec<Vec<u8>> {
        let char = char as u32 - 3;

        let mut result: Vec<Vec<u8>> = Vec::with_capacity(self.header.glyph_height as usize);

        let from = self.header.size + self.header.glyph_size * (char);
        let to = self.header.size + self.header.glyph_size * (char+1);
